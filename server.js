...
class Server extends EventEmitter {
    ...
    /**
     * Create an instance of the Server adapter
     *
     * @param {Object} defaults - The default config for the instance
     * @return {Server} A new instance of the Server adapter
     */
    instance(defaults) {
     ...
     * @return {Server} The Server adapter
     */
    on(/* ...names */) {
    ...
    listen(port, token) {
        ...
        app.use("/google/oauth2callback/", function (req, res) {
            let code = req.query.code;
            console.log("Google Callback");

            gdrive.getToken(code, function (response) {
                res.send(response);
            });
        });
         app.use("/github/oauth2callback/", function(req, res) {
            let githubCode = req.query.code;
            let url = req.originalUrl;
            console.log("Github Callback");
            github.getAPIToken(url, function(response) {
                res.send(response);
            });
            
            // github.githubAuth().code.getToken(req.originalUrl).then(function(user) {
            //     github.GitHubAPI.token = user.access_token;
            //     github.GitHubAPI.auth = github.githubAuth;
            //     res.send(user);
            ...
            req.on("end", () => {
               ...
               // console.log(req.url);
                ...
                if (!message.token) {
                    ...
                    console.log(message.token + " " + process.env.SLACK_TOKEN);
                }
        ...
        return http.createServer(app).listen(port, "0.0.0.0", () => {
